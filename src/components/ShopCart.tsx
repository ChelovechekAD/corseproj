...
function ShopCart() {
    ...
    return (
        <div className='shop-cart'>
            ...
            {orderCartStore.items.length > 0 && (<p className='summa'>Сумма: {new Intl.NumberFormat().format( orderCartStore.sumPrice)}$</p>)}
            {orderCartStore.items.length > 0 && (<Link to='/order' className='do-order' onClick={()=>{store.setPath("/order"); orderCartStore.setCartOpen()}}>Оформить</Link>) }
